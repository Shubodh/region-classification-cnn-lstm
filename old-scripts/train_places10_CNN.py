...
def main():
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            model.cuda()
            print("final layer replaced with 10 neurons")
            print model
        else:
    ...
    print("batch size is {bat}".format(bat=args.batch_size))
    ...
    params_Nmin1 = []
    for child in list(model.module.cuda().children())[:-1]:
        params_Nmin1.extend(list(child.parameters()))

    optimizer = torch.optim.SGD(
            [
                {"params": model.module.fc.parameters(), "lr": 0.1},
                {"params": params_Nmin1, "lr": 0.01},
            ],
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    info = { 'train_loss': losses.avg, 'train_accuracy': top1.avg }
    ...
    for tag, input_var in info.items():
        logger.image_summary(tag, input_var, epoch) 
...
#            print("the length of pred list is {pred}".format(pred = len(pred_list)))
    ...
    # TENSORBOARD LOGGING
    # 1. Log scalar values (scalar summary)
    info = { 'validation_loss': losses.avg, 'validation_accuracy': top1.avg }
